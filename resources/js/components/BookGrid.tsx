...
import { Book, LaravelCategory } from '../types';
...
const BookCard: React.FC<{ book: Book }> = ({ book }) => {
    return (
        <div className="group cursor-pointer">
            ...
            >
                {/* Cover Image or Artistic Placeholder */}
                {book.coverPhoto ? (
                    <img 
                        src={`/storage/${book.coverPhoto}`} 
                        alt={book.title} 
            ...
            <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 pt-1">
                    ...
                    {book.originalPrice && (
                        <p className="text-xs text-gray-400 line-through">Rp {Number(book.originalPrice).toLocaleString('id-ID')}</p>
                    )}
                </div>
            </div>
        </div>
...
const BookGrid: React.FC<BookGridProps> = ({ books, categories }) => {
    return (
                        ...
                        {categories.map(category => (
                            <li key={category.id}>
                                <a href="#" className="group flex items-center justify-between text-sm text-gray-500 hover:text-gray-900 transition-colors">
                                    <span className="border-b border-transparent group-hover:border-gray-900 pb-0.5 transition-all">{category.name}</span>
                                    <span className="text-xs text-gray-300">{category.books_count || 0}</span>
                                </a>
                            </li>
                        ))}
                    </ul>
